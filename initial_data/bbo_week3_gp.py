...
def get_local_noise(func_idx):
    # More exploitation for strong/promising functions
    if func_idx == 4:   # Function 5
        return 0.01
    if func_idx in [6, 7]:  # Functions 7 and 8
        return 0.03
    if func_idx in [1, 5]:  # Functions 2 and 6
        return 0.04
    return 0.06

def get_xi(func_idx):
    # More exploration for difficult functions
    if func_idx in [0, 3]:  # Functions 1 and 4
        return 0.08
    if func_idx == 1:       # Function 2
        return 0.03
    return 0.01

def propose_query_gp(X, y, func_idx, n_candidates=20000):
    d = X.shape[1]
    ...
    # Candidate strategy:
    # Functions 1 and 4 => more global exploration
    # Function 5 => heavy local exploitation
    if func_idx == 0:  # Function 1
        n_global = 18000
    elif func_idx == 3:  # Function 4
        n_global = 16000
    elif func_idx == 4:  # Function 5
        n_global = 4000
    else:
        n_global = 12000
...
def main():
    ...
    for i, func_name in enumerate(FUNCTION_FOLDERS):
        ...
        # Append Round 1 and Round 2
        X = np.vstack([X_old, round1_inputs[i], round2_inputs[i]])
        y = np.append(y_old, [round1_outputs[i], round2_outputs[i]])
    ...
    with open("week3_queries.txt", "w", encoding="utf-8") as f:
        for line in lines:
            f.write(line + "\n")

    print("\nSaved to week3_queries.txt")
