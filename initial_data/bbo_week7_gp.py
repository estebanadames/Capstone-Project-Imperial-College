...
week_outputs = [
    [2.6752879910742468e-09, 2.6752879910742468e-09, -1.560646704467778e-117, 1.2411689730836742e-151, 2.8556640528793443e-181, 2.8556640528793443e-181],
    [0.03838400221215603, 0.5285960143922023, 0.5086800893414688, 0.08359001917456062, -0.014987991658531197, 0.39928503396910486],
    [-0.09560907240309474, -0.08700196832088761, -0.012630818813151479, -0.06816775168805957, -0.11790008081891142, -0.060846422227520666],
    [-11.923018749116093, -0.8835934440569733, 0.6285556396489267, 0.5312263478605925, -0.023165133710903785, 0.5469944080570142],
    [1275.2685405563973, 3249.3430704956318, 3643.287564441108, 1975.2659195490683, 3867.0650282755832, 4167.294359720726],
    [-0.7363719843000791, -0.46412435211066894, -0.48783256521310503, -0.5991304321604517, -0.589937785370754, -0.4814131923770772],
    [0.1660141105990867, 1.3016764549502793, 1.325428742931525, 1.223668225491426, 1.6644973432756731, 1.7934076476132588],
    [9.6407935213586, 9.9293867977869, 9.8975309817194, 9.9469856420036, 9.8158126377346, 9.9006560172534],
]
...
def propose_query(X, y, func_idx):
    ...
    if func_idx == 4:      # Function 5: strong exploitation
        n_global = 2000
        local_noise = 0.010
        xi = 0.002
    elif func_idx == 6:    # Function 7: strong local refinement
        n_global = 4000
        local_noise = 0.018
        xi = 0.004
    elif func_idx == 7:    # Function 8: plateau, controlled refinement
        n_global = 7000
        local_noise = 0.025
        xi = 0.008
    elif func_idx == 0:    # Function 1: still dead-flat
        n_global = 30000
        local_noise = 0.090
        xi = 0.180
    elif func_idx == 3:    # Function 4: good region, refine
        n_global = 6000
        local_noise = 0.025
        xi = 0.006
    else:
...
def main():
    ...
    with open("week7_queries.txt", "w", encoding="utf-8") as f:
        for line in lines:
            f.write(line + "\n")

    print("\nSaved to week7_queries.txt")
