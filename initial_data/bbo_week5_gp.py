...
def get_local_noise(func_idx):
    # More stable exploitation where we have strong signal
    ...
    if func_idx in [1, 2, 5]:  # Functions 2, 3, 6
        return 0.035
    return 0.070  # Function 1 still needs exploration

def get_xi(func_idx):
    # More exploration for functions with weak/unclear signal
    if func_idx == 0:   # Function 1
        return 0.150
    if func_idx == 1:   # Function 2
        return 0.060
    if func_idx == 2:   # Function 3
        return 0.030
    if func_idx == 5:   # Function 6
        return 0.030
    return 0.010

def propose_query_gp(X, y, func_idx, n_candidates=30000):
    d = X.shape[1]

    kernel = (
        ConstantKernel(1.0, (1e-5, 1e3))
        * Matern(length_scale=np.ones(d), length_scale_bounds=(1e-2, 20.0), nu=2.5)
    ...
    # Candidate allocation by function behavior
    if func_idx == 0:      # F1: explore hard
        n_global = 26000
    elif func_idx == 1:    # F2: explore more because recent drop
        n_global = 22000
    elif func_idx == 4:    # F5: exploit hard
        n_global = 2000
    elif func_idx == 7:    # F8: mostly exploit, still some exploration
        n_global = 8000
    else:
        n_global = 14000
...
def main():
    ...
    with open("week5_queries.txt", "w", encoding="utf-8") as f:
        for line in lines:
            f.write(line + "\n")

    print("\nSaved to week5_queries.txt")
